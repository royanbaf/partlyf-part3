...
<head>
    ...
    .luxury-card:hover {
        ...
        box-shadow: 0 10px 25px -5px rgba(245, 158, 11, 0.08), 0 1px 2px rgba(255, 255, 255, 1) inset;
    }
    ...
    .customer-pagination-container nav>div:first-child {
        display: none;
    }

    .customer-pagination-container nav>div:last-child {
        display: flex;
    ...
    .customer-pagination-container .page-link,
    .customer-pagination-container span[aria-current="page"]>span,
    .customer-pagination-container nav span.relative {
        width: 38px;
        height: 38px;
        display: flex;
        align-items: center;
    ...
    .customer-pagination-container a.page-link,
    .customer-pagination-container span.page-link {
        background-color: #ffffff !important;
        color: #64748b !important;
        border: 1px solid #e2e8f0 !important;
    }

    .customer-pagination-container a.page-link:hover {
        border-color: #f59e0b !important;
    ...
    .customer-pagination-container a[rel="next"],
    .customer-pagination-container a[rel="prev"] {
        font-size: 10px;
...
<body
    class="bg-[#f8fafc] font-sans text-slate-700 h-screen overflow-hidden flex selection:bg-amber-100 selection:text-amber-900">
    ...
    <div class="flex-1 flex flex-col h-screen overflow-hidden relative z-10">
        ...
        <header class="h-20 glass-header flex items-center justify-between px-8 flex-shrink-0 z-50 sticky top-0">
            ...
            <div class="relative w-full max-w-3xl flex-grow z-[100]">
                <form id="ai-search-form" action="{{ route('customer.dashboard') }}" method="GET" class="relative">
                    <input type="text" name="search" id="ai-search-input" value="{{ $search ?? '' }}"
                        placeholder="Ketik keluhan motor (Misal: 'Tarikan berat' atau 'Rem blong')..."
                        autocomplete="off"
                    ...
                    <button type="submit"
                        class="absolute left-4 top-2.5 text-slate-400 hover:text-amber-600 transition-colors">
            ...
            <div class="flex items-center gap-6 ml-8">
                <a href="{{ Auth::check() ? route('customer.wishlist') : route('login') }}"
                    class="relative text-slate-400 hover:text-rose-500 transition-all hover:scale-110">
                    <i class="fa-solid fa-heart text-xl"></i>
                </a>
                <a href="{{ Auth::check() ? route('customer.cart') : route('login') }}"
                    class="relative text-slate-400 hover:text-amber-500 transition-all hover:scale-110">
                    <i class="fa-solid fa-cart-shopping text-xl"></i>
                </a>
                <div class="h-6 w-px bg-slate-200"></div>
                @auth
                <a href="{{ route('customer.profile') }}"
                    class="w-9 h-9 bg-gradient-to-br from-amber-400 to-amber-500 text-slate-900 rounded-full flex items-center justify-center font-black text-sm transition-transform hover:scale-110"
                    style="box-shadow: 0 4px 12px rgba(245,158,11,0.3);">
                    {{ substr(Auth::user()->name, 0, 1) }}
                </a>
                @else
                <a href="{{ route('login') }}"
                    class="text-xs font-bold text-amber-700 bg-amber-500/10 px-4 py-2 rounded-full border border-amber-500/20 hover:bg-amber-500 hover:text-white transition-all">Masuk</a>
                @endauth
            </div>
        </header>
        ...
        <main class="flex-1 overflow-y-auto p-8 relative">
            <div class="max-w-[1200px] mx-auto">
                ...
                {{-- BANNER PROFIL UTAMA --}}
                <div id="hero-profile-banner"
                    class="mb-10 p-7 rounded-3xl bg-white border border-slate-200/80 relative overflow-hidden flex flex-wrap gap-6 justify-between items-center shadow-sm">
                    <div class="flex items-center gap-5">
                        <div
                            class="w-16 h-16 rounded-full bg-gradient-to-br from-amber-400 to-amber-500 text-slate-900 text-2xl font-black flex items-center justify-center shadow-sm">
                            {{ substr(Auth::user()->name ?? 'P', 0, 1) }}
                        </div>
                        <div>
                            <p class="text-xs text-slate-400 font-medium">Selamat Datang di Partlyfe,</p>
                            <h2 class="text-2xl font-black text-slate-900">{{ Auth::user()->name ?? 'Pelanggan Setia' }}
                            </h2>
                            <p class="text-xs text-amber-700 mt-1 font-semibold flex items-center gap-1.5"><i
                                    class="fa-solid fa-coins"></i> {{ Auth::user()->loyalty_points ?? 0 }} partlyfe
                                points</p>
                        </div>
                    </div>
                    ...
                    <div class="flex gap-4">
                        @foreach([
                        ['fa-basket-shopping', route('customer.cart'), (\App\Models\Cart::where('user_id',
                        Auth::id())->count() ?? 0) . ' Barang', 'Keranjang'],
                        ['fa-receipt', route('customer.transactions'), (\App\Models\Transaction::where('user_id',
                        Auth::id())->count() ?? 0) . ' Transaksi', 'Riwayat'],
                        ['fa-robot', route('customer.ai-chat'), 'Tanya AI', 'Mekanik']
                        ] as $info)
                        <a href="{{ $info[1] }}"
                            class="p-4 rounded-2xl border border-slate-100 bg-slate-50 flex flex-col items-center text-center w-28 hover:border-amber-200 hover:bg-amber-50 transition-all group">
                            <i
                                class="fa-solid {{ $info[0] }} text-xl text-amber-500 mb-2.5 transition-transform group-hover:scale-110"></i>
                ...
                <div id="main-catalog-section">
                    ...
                    <div id="categories-filter-box"
                        class="mb-8 p-6 rounded-3xl bg-white border border-slate-200/80 shadow-sm">
                        <p
                            class="text-xs text-slate-400 font-bold uppercase tracking-wider mb-4 flex items-center gap-2">
                        ...
                        <div class="flex gap-3 overflow-x-auto pb-1 no-scrollbar">
                            <a href="{{ route('customer.dashboard') }}"
                                class="px-5 py-2.5 rounded-xl border border-slate-200 bg-white text-xs font-bold text-slate-500 transition-all hover:border-slate-400 hover:text-slate-800 flex-shrink-0 {{ !$search ? 'category-btn-active' : '' }}">
                                Semua Suku Cadang
                            </a>
                            @foreach($categories as $cat)
                            <a href="{{ route('customer.dashboard', ['search' => $cat->name]) }}"
                                class="px-5 py-2.5 rounded-xl border border-slate-200 bg-white text-xs font-bold text-slate-500 transition-all hover:border-slate-400 hover:text-slate-800 flex-shrink-0 {{ isset($search) && strtolower($search) === strtolower($cat->name) ? 'category-btn-active' : '' }}">
                                {{ $cat->name }}
                            </a>
                            @endforeach
                        </div>
                    ...
                    <div class="flex justify-between items-center mb-6">
                        <p id="catalog-title" class="text-xs text-slate-400 font-bold uppercase tracking-wider">
                            Daftar Komponen Presisi <span class="text-slate-600">(Total {{ $products->total() }}
                                SKU)</span>
                        </p>
                    </div>
                    ...
                    <div id="products-grid-container" class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
                        ...
                        $isOutofStock = $prod->current_stock <= 0; @endphp <div
                            class="luxury-card rounded-2xl flex flex-col overflow-hidden relative {{ $isOutofStock ? 'opacity-80' : '' }}">
                            @if($isOutofStock)
                            <div
                                class="absolute top-3 left-3 bg-rose-500 text-white text-[9px] font-black px-2.5 py-1 rounded shadow-sm z-20">
                                STOK HABIS
                            </div>
                            @elseif($prod->cashback_percent > 0)
                            <div
                                class="absolute top-3 left-3 bg-rose-500 text-white text-[9px] font-black px-2 py-0.5 rounded z-20">
                                Cashback {{ $prod->cashback_percent }}%
                            </div>
                            @endif

                            <a href="{{ route('product.detail', $prod->id) }}"
                                class="h-44 bg-white flex items-center justify-center p-4 border-b border-slate-100 relative overflow-hidden group-hover:scale-105 transition-transform duration-500">
                                @if($prod->images && $prod->images->isNotEmpty())
                                <img src="{{ asset('storage/products/' . basename(optional($prod->images->first())->image_path ?? 'default.png')) }}"
                                    alt="{{ $prod->name }}"
                            ...
                            <div class="p-4 flex flex-col flex-grow bg-white">
                                <p class="text-[9px] text-amber-600 font-black uppercase tracking-widest mb-1">
                                    {{ $prod->brand }}</p>
                                <a href="{{ route('product.detail', $prod->id) }}"
                                    class="text-sm font-bold text-slate-800 line-clamp-2 leading-snug hover:text-amber-600 transition-colors mb-4 h-10">
                                    {{ $prod->name }}
                                </a>

                                <div
                                    class="mt-auto pt-3 border-t border-slate-100 flex justify-between items-center gap-3">
                                    <p class="font-black text-slate-900 text-base">Rp
                                        {{ number_format($price->price ?? 0, 0, ',', '.') }}</p>
                                    ...
                                    <form action="{{ route('wishlist.toggle', $prod->id) }}" method="POST">
                                        ...
                                        <button type="submit" title="Simpan ke Wishlist"
                                            class="w-8 h-8 rounded-xl bg-rose-50 text-rose-500 flex items-center justify-center hover:bg-rose-500 hover:text-white transition-all shadow-sm border border-rose-100">
                                            <i class="fa-solid fa-heart text-xs"></i>
                                        </button>
                                    </form>
                                    @else
                                    <a href="{{ route('login') }}" title="Simpan ke Wishlist"
                                        class="w-8 h-8 rounded-xl bg-rose-50 text-rose-500 flex items-center justify-center hover:bg-rose-500 hover:text-white transition-all shadow-sm border border-rose-100">
                                        <i class="fa-solid fa-heart text-xs"></i>
                                    </a>
                                    @endauth
                                    ...
                                    <form action="{{ route('cart.add', $prod->id) }}" method="POST">
                                        ...
                                        <button type="submit" title="Tambah ke Keranjang"
                                            class="w-8 h-8 rounded-xl bg-amber-500 text-slate-900 flex items-center justify-center hover:bg-amber-600 transition-all shadow-sm font-bold text-xs">
                                            <i class="fa-solid fa-cart-plus text-xs"></i>
                                        </button>
                                    </form>
                                    @else
                                    <a href="{{ route('login') }}" title="Tambah ke Keranjang"
                                        class="w-8 h-8 rounded-xl bg-slate-100 text-slate-500 flex items-center justify-center hover:bg-amber-500 hover:text-white transition-all shadow-sm">
                                        <i class="fa-solid fa-cart-plus text-xs"></i>
                                    </a>
                                    @endauth
                                    @endif
    ...
    async function fetchAiDirectResults(query) {
        try {
            ...
            if (result.data && result.data.length > 0) {
                ...
                catalogTitle.innerHTML =
                    `<i class="fa-solid fa-sparkles text-amber-500 mr-1"></i> Rekomendasi Sinar Jaya Motor untuk Keluhan <span class="text-amber-700 font-bold">"${result.interpreted_as.toUpperCase()}"</span> (${result.data.length} SKU):`;
    ...
    async function beliLangsung(productId) {
        try {
            ...
            if (!response.ok) {
                ...
                alert(
                `Server Menolak (Status HTTP ${response.status}). Pastikan Anda telah login dengan akun b2c.`);
                document.body.style.cursor = 'default';
                return;
            }
